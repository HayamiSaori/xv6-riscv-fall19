...
bd_malloc(uint64 nbytes)
{
  ...
  bit_set_xor(bd_sizes[k].alloc, blk_index(k, p));
  for(; k > fk; k--) {
    ...
    bit_set_xor(bd_sizes[k-1].alloc, blk_index(k-1, p));
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_set_xor(bd_sizes[k].alloc, bi);
    // if (bit_isset(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
    if(bit_isset_xor(bd_sizes[k].alloc, buddy)) {
...
bd_initfree_pair(int k, int bi, void *start, void *end){
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bit_isset_xor(bd_sizes[k].alloc, bi) == 1 /*!=  bit_isset(bd_sizes[k].alloc, buddy)*/) {
    // one of the pair is free
    free = BLK_SIZE(k);
    // if(bit_isset(bd_sizes[k].alloc, bi))
    if(addr(k,buddy) >= start && addr(k,buddy) <= end)
...
bd_initfree(void *bd_left, void *bd_right, void *start, void *end) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {   // skip max size
    ...
    free += bd_initfree_pair(k,left,start,end);
    if(right <= left)
      continue;
    // free += bd_initfree_pair(k, right);
    free += bd_initfree_pair(k,right,start,end);
  }
  return free;
}
